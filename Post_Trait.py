...
class POSTRAIT:
    ...
    def __init__(self,Path_CFG,label="EnVar(2)",color="k",CFG_file_name = "CONFIG.pkl", Path_Src = None) :
    ...
    def update_path_src_of_CFG(self):
        d_glob = os.getcwd()
        os.chdir(self.Path_src)

        loc_d              = os.getcwd()
        self.Old_Path_Src  = self.CFG.Path_Src
        self.CFG.Path_Src  = loc_d + '/'
        self.CFG.INIT_FOLDER()
        os.chdir(d_glob)
        ...
        if not self.CFG.Use_DL:
            ...
            for iter in range(Nb_iter) :
                ...
                if N_ens_HF < Nb_ens :
                    ...
                    if self.CFG.Param_Dim <2 :
                        Param_ens_DL = Param_ens_DL.reshape( (len(Param_ens_DL),1) )
                    if len(Param_ens_DL.shape) <2 and self.CFG.Param_Dim >= 2 :
                        Param_ens_DL = Param_ens_DL.reshape( (1,self.CFG.Param_Dim ) )
